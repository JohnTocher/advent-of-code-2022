""" Advent of code 2022 - Puzzle 11
    ...
    John Tocher
    Solution to puzzle 11 part 2
"""
1
INPUT_FILE_NAME = "puzzle_11_input.txt"


def monkey_inspection_result(
    worry_level, monkey_details, common_divisor, print_verbose=False
):
    ...
    final_value = final_value % common_divisor  # Don't let it grow too large

    divisor = monkey_details["divisor"]
    assert divisor > 0, f"Unexpected divisor: {divisor}"

    # bored_value = int(final_value / 3)
    bored_value = final_value  # Change for part 2
...
def print_monkey_data(all_data, just_inspections=False):
    ...
    for monkey_num, monkey_details in all_data.items():
        ...
        if just_inspections:
            ...
            line_detail = line_list.ljust(50)
        line_insp = f"{monkey_details['inspections']}"
        print(f"{line_label} - {line_detail} Insp:{line_insp}")
...
def get_common_divisor(all_data):
    """Returns the product of all of the constants used for divisor checks"""

    common_divisor = 1

    for monkey_data in all_data.values():
        common_divisor *= monkey_data["divisor"]

    return common_divisor


group_data = read_monkey_data(verbose_output=False)
common_divisor = get_common_divisor(group_data)  # Multiple of modulo for all monkeys
print(f"Common divisor is: {common_divisor}")

for round_count in range(1, 10001):
    ...
    for monkey_num, monkey_data in group_data.items():
        ...
        for each_item in monkey_data["items"]:
            ...
            target, worry_value = monkey_inspection_result(
                each_item, monkey_data, common_divisor, print_verbose=False
            )
    ...
    print_monkey_data(group_data, just_inspections=True)
